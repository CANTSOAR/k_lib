...
def download_pdf(arxiv_id, save_dir):
    """
    Downloads the PDF from arXiv and saves it to the paper's folder.
    Returns the path to the PDF file, or None if download failed.
    """
...
def fetch_volatility_papers():
    ...
    for i, paper in enumerate(results):
        ...
        # Download PDF (always try, even if source fails)
        pdf_path = download_pdf(paper_id, base_dir)
        ...
        if source_folder:
            ...
            if main_tex_file:
                ...
                paper_meta = {
                    ...
                    "pdf_path": pdf_path,  # New: Path to downloaded PDF
                    "arxiv_url": f"https://arxiv.org/abs/{paper_id}",  # New: Link to arXiv page
                    "url": paper.pdf_url
                }
